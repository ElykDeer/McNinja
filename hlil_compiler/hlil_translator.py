  ...
  if isinstance(hlil_instr, bn.highlevelil.HighLevelILInstruction):
    ...
    if hlil_instr.operation == ops.HLIL_CALL_SSA:
      ...
      for operand in hlil_instr.params:
        traverse_il_block(parser, operand, builder, current_function_vars, new_args)
    ...
    for entry in hlil_instr:
      traverse_il_block(parser, entry, builder, current_function_vars, current_args)
  else:
...
def translate_function(parser, func, ir_func):
  function_locals = {}
  for bb in func.high_level_il.ssa_form:
    ir_bb = ir_func.append_basic_block()
    builder = ir.IRBuilder(ir_bb)
    for instr in bb:
      traverse_il_block(parser, instr, builder, function_locals, [])
